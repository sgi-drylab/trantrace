<template>
  <div id="projectMain" class="clearfix">
    ...
    <div class="addSearch clearfix">
      <div id="searchCon">
        <el-form :inline="true" :model="searchForm" ref="searchForm" :rules="rules" label-position="left" label-width="120px" class="demo-form-inline" size="small" @submit.native.prevent>
          <el-form-item label="Project Name:" prop="product_name">
            <el-input v-model="searchForm.product_name" placeholder="enter project name"></el-input>
          </el-form-item>
          <el-form-item label="Deadline:" prop="deadline">
            <el-date-picker v-model="searchForm.deadline" type="date" :default-value="new Date()" value-format="yyyy-MM-dd" placeholder="select deadline"></el-date-picker>
          </el-form-item>
    ...
    <div class="showUserInfo">
      <el-table ref="ProjectTable" :data="projectListTable" v-loading="loading" style="width: 100%" @row-dblclick="rowDblClick" @sort-change="sortChange">
...
export default {
  mounted(){
    ...
    // }).then(data=>{
      ...
      this.$http.post("/api/product/list",qs.stringify({page:this.$route.query.page||1,count:this.$route.query.count||1,product_name:this.$route.query.name,priority:this.$route.query.priority,deadline:this.$route.query.deadline})).then(response=>{
